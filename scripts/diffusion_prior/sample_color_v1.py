...
from diffusion_prior_guided_diffusion import logger
from diffusion_prior_guided_diffusion.script_util_v1 import (
    NUM_CLASSES,
...
def main():
    ...
    def color_cond_fn(x, t, y=None, x_lr=None, sample_noisy_x_lr=False, diffusion=None, sample_noisy_x_lr_t_thred=None):
        ...
        with th.enable_grad():
            ...
            if not x_lr is None:
                ...
                loss = - mse * args.img_guidance_scale - loss_col*500 # move xt toward the gradient direction
    ...
    for i, data in enumerate(dataloader):
        if args.use_img_for_guidance:
            ...
            cond_fn = lambda x,t,y : color_cond_fn(x, t, y=y, x_lr=image_lr, sample_noisy_x_lr=args.sample_noisy_x_lr, diffusion=diffusion, sample_noisy_x_lr_t_thred=args.sample_noisy_x_lr_t_thred)
        else:
            image, label = data
...
def create_argparser():
    ...
    parser.add_argument("--save_dir", default='/nvme/feiben/GDP/generate_images/generated_image_color_v1', type=str, help='the directory to save the generate images')
    ...
    parser.add_argument("--dataset_path", default='/nvme/lustre/feiben/DDPM_Beat_GAN/evaluations/precomputed/biggan_deep_imagenet64.npz', type=str, help='path to the generated images. Could be an npz file or an image folder')
    ...
    parser.add_argument("--base_samples", default='/nvme/feiben/DDPM_Beat_GAN/scripts/imagenet_dataloader/gray_resolution_256.npz', type=str, help='the directory or npz file to the guidance imgs. This folder should have the same structure as dataset_path, there should be a one to one mapping between images in them')
